...
5. Token is accepted → Authentication bypass

## 🚀 Step-by-Step Exploitation
### 1️⃣ Generate an Attacker-Controlled RSA Key
In Burp Suite:
...
- Public key → Injected via jwk

### 2️⃣ Intercept the Admin Request
In Repeater:
...
⚠️ This alone will not work — the token must be re-signed.

### 4️⃣ Inject the JWK into the JWT Header
In the JWT Editor tab:
1. Click Sign
2. Select your RSA key
3. Uncheck “Don’t modify header”
...
✔ The public key inside jwk matches the private key used to sign the token.

### 5️⃣ Send the Forged Request
...
### ➡️ JWT security is completely defeated
...
- The server does **not verify the JWT signature at all**.

### That means:
- You can modify the payload
- Leave the signature untouched
- Server will still trust the token

---

## Where you are so far (recap)

- Found JWT in `/my-account`
- Decoded payload
- Changed `sub` from `wiener` → `administrator`
- Confirmed `/admin` is admin-only

---

## Critical Step (the actual bypass)

### ❗ Do NOT re-sign the token
...
- Change the secret
- Change alg
Just click Apply changes in Burp Inspector

### Why?
Because the server never checks whether the signature matches the payload.
...
### Delete Carlos (or the required user)
Once inside /admin:
- Click Delete carlos
- 🎯 Lab solved

### Why This Works (interview / report-ready explanation)
### Vulnerability
...
- Reject tokens with:
    - Invalid signatures
    - Tampered payloads
- Use trusted JWT libraries correctly
...
- Edit claim
- Apply changes
- Never touch the signature unless the lab explicitly mentions:
    -alg: none
